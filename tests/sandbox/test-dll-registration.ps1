...
# Writes results to $env:TEMP\DraftHorse-registration-test.log

$ErrorActionPreference = "Stop"
$ProjectRoot = "C:\DraftHorse"
$DllPath = "$ProjectRoot\src\interceptor\build\bin\DraftHorse.dll"
...
try {
    $regPath = "HKLM:\SOFTWARE\Clients\Mail\DraftHorse"
    New-Item -Path $regPath -Force | Out-Null
    New-ItemProperty -Path $regPath -Name "(Default)" -Value "DraftHorse" -PropertyType String -Force | Out-Null
    New-ItemProperty -Path $regPath -Name "DLLPath" -Value $DllPath -PropertyType String -Force | Out-Null
    Set-ItemProperty -Path "HKLM:\SOFTWARE\Clients\Mail" -Name "(Default)" -Value "DraftHorse" -Force
...
$goMapiKey = "HKLM:\SOFTWARE\Clients\Mail\DraftHorse"
...
if ($defaultMail -ne "DraftHorse") {
    Log "FAILED: Default mail client is '$defaultMail', expected 'DraftHorse'"
    exit 1
}
Log "OK: Default mail client = DraftHorse"
